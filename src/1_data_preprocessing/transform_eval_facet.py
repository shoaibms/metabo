...
def calculate_cv(series: pd.Series) -> float:
    ...
    Returns:
        float: Coefficient of Variation as a percentage, or np.nan if mean is zero.
    """
    mean = np.mean(series)
    if mean == 0:
        return np.nan
    return (np.std(series) / mean) * 100
...
def calculate_rmad(series: pd.Series) -> float:
    ...
    Returns:
        float: relative Median Absolute Deviation ratio, or np.nan if median is zero.
    """
...
def load_data(original_path: str, transformed_paths: Dict[str, str]) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    """
...
def _create_metric_df(metric_name: str, orig_metric: pd.Series, trans_metric: pd.Series, trans_name: str) -> pd.DataFrame:
    """Helper function to create a dataframe for a given metric."""
    df = pd.DataFrame({
        'Original': orig_metric,
        'Transformed': trans_metric
    }).reset_index().melt(id_vars='index', var_name='Type', value_name=metric_name)
    df['Transformation'] = trans_name
    return df

def prepare_data_for_plotting(original_df: pd.DataFrame, transformed_dfs: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    for name, df in transformed_dfs.items():
        ...
        cv_df = _create_metric_df('CV', orig_cv, trans_cv, name)
        rmad_df = _create_metric_df('rMAD', orig_rmad, trans_rmad, name)
...
def plot_facet_grid(df: pd.DataFrame, metric: str, font_sizes: Dict[str, int]) -> None:
    """
    ...
    Args:
        ...
        font_sizes (dict): Dictionary containing font size specifications
    """
    ...
    for ax in g.axes.flat:
        if ax:
            ax.set_xlabel('Data Type', fontsize=font_sizes['xlabel'])
    ...
    g.fig.suptitle(f'Violin Plot of {metric}', fontsize=font_sizes['title'])
    plt.show()

def main() -> None:
    """Main function to run the data transformation evaluation."""
    # Define file paths
    original_path = r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if.csv"
    transformed_paths = {
        'anscombe': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_anscombe.csv",
        'asinh': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_asinh.csv",
        'boxcox': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_boxcox.csv",
        'glog': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_glog.csv",
        'log': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_log.csv",
        'sqrt': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_sqrt.csv",
        'yeojohnson': r"C:\Users\USER\Desktop\data_chem\data\transformation\n_l_if_yeojohnson.csv"
    }
    ...
    plot_facet_grid(cv_df, 'CV', FONT_SIZES)
    plot_facet_grid(rmad_df, 'rMAD', FONT_SIZES)
